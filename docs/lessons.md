...
**Evidence:** Plan C1 `bd7f574` — `resize_window` returned success twice while `innerWidth` never left 1280. · Certification grants verification, 2026-08-19 — a PostgREST `PATCH` with `Prefer: return=minimal` answered **204** where RLS had silently filtered the statement to zero rows. That status is returned identically whether the update touched every matching row or none, so it cannot distinguish the two outcomes the probe existed to tell apart. Reading the row back settled it (unchanged; no row anywhere held the sentinel). The probe that answers honestly is `Prefer: count=exact` **alone** — the same call then returns 204 with `Content-Range: */0`, which is the affected-row count and needs no SELECT privilege. `return=representation` is a **trap** on any column-grant-restricted table: it answers 403 `42501` because the default representation is `*` and the column grant forbids reading it back — identical in appearance to a refused write, and not one. Measured across all four variants with the grants restored; naming only granted columns in `select=` turns that 403 into 200 `*/0` `[]`, which is what proves it was a read denial. The 204 was one sentence away from being reported as a critical privilege-escalation finding — and the first draft of this very entry prescribed the `return=representation` probe, i.e. shipped the conflation inside the warning against it.
