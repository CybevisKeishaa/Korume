...
**Evidence:** Plan C1 — an unmeasured "26" went into a code-reviewer's prompt and was echoed in its report. The round-2 dispatch instead told the reviewer to measure, and it promptly caught the next stale count. · Screen Registry Phase 3 Stage 1 — **the controller seeded three false propositions, none of them a number.** (a) A dispatch and the plan both said a mutation "will also fail `R12` because there are now 14 keys" — false; the field had been backfilled onto every row in Task 1, so the mutation changed a *value*, not the key set, and `R12` correctly stayed green. The implementer caught it unprompted and said so. (b) A ruling asserted the full suite "exceeds the 600s cap" — false; it runs in ~72s and the real cause was output volume (`L-035`). (c) A fix-round message asserted the batch was "the 13 previously-uncaptured frames **plus `347:6277`**" — false; `347:6277` was already among the 13 and the fourteenth member was a different node. The implementer transcribed (c) faithfully and the resulting text had to be fixed twice. **What generalises:** (a) and (b) were caught, (c) was not — and the difference is that (a) and (b) were checkable from inside the task, while (c) required reading a document the agent had no reason to open. Seed only claims the receiving agent can falsify from its own working set; anything else must be handed over as "verify this, report disagreement".
...
**Evidence:** Screen Registry Phase 3 Stage 1 — two full-suite runs were killed with no `EXIT=` line ever written. The controller ruled that the suite "exceeds the 600s cap" on a subagent's Bash tool and reassigned the run to itself. The rerun with `--reporter=dot` completed in **71.77s** — an order of magnitude inside the cap the ruling had blamed. The ruling's conclusion was harmless but its stated reason was false, and it was corrected rather than left to be inherited by later tasks (`L-012`). The separately true fact worth keeping: a subagent's Bash tool does cap at 600s, so any command genuinely approaching it belongs in the controller's seat or in a background run.
...
**Evidence:** Screen Registry Phase 3 Stage 1 — a backfill script added one field to all 81 registry entries and left `lib/product/screen-registry.ts` permanently ` M`. An implementer reported it as "a stale mtime in git's stat cache"; the real cause was the LF/CRLF flip, and the file at that moment was byte-identical to `HEAD` by hash. Left unfixed it would have landed inside Task 5, the one task that edits that same 1487-line file heavily — turning a 181-line reviewable diff into a whole-file rewrite.
