...
**Evidence:** Plan C1 `bd7f574` — `resize_window` returned success twice while `innerWidth` never left 1280. · Certification grants verification, 2026-08-19 — a PostgREST `PATCH` with `Prefer: return=minimal` answered **204** where RLS had silently filtered the statement to zero rows. That status is returned identically whether the update touched every matching row or none, so it cannot distinguish the two outcomes the probe existed to tell apart. Reading the row back settled it (unchanged; no row anywhere held the sentinel). The probe that answers honestly asks for `Prefer: count=exact`, whose `Content-Range` carries the affected-row count — with two traps around it that live in `mem:project_status` § Key gotchas, this file's scope being process rather than provider mechanics (see § Scope above). **The part that generalises:** this entry's first draft prescribed a probe that could not itself distinguish the two outcomes — the conflation shipped *inside the warning against it*. The second got the probe right and the qualifiers around it wrong. The third got a qualifier wrong again — this time in the recipe's own home, about the signal the probe reads. Each wave's defect was caught by the review of that wave, never by its author. When a lesson is about a signal being ambiguous, its own correction is the most likely place for a fresh ambiguous claim to hide, and the claim will be in a scope word rather than in the substance.
...
**Evidence (cont.):** L9b Plan 1 whole-branch review, 2026-08-22 — three at once, and the third is the interesting one. (a) A test asserting `href` `stringContaining("/settings/privacy/memory")` was green *because it checked the wrong thing*: no `page.tsx` existed at that route, and with no `not-found.tsx` in the app the row landed on Next's default 404. Asserting the STRING and asserting the DESTINATION are different tests. (b) A `queryByRole("alert")).not.toBeInTheDocument()` sat in a sequence where no submit occurred, so no alert could exist either way — it survived two review passes. (c) A guard titled *"never focuses itself"* was flagged as inert on the grounds that no mutation turned it red. It was not inert; **the mutation for a guard over an ABSENCE is re-introducing the code that was removed**, not perturbing code that is there. Re-adding the mount-time focus effect the branch had shipped once turned that assertion, and only that one, red. Reaching for the usual "break the implementation" mutation and finding nothing to break is not evidence that a guard is worthless — it is evidence the wrong mutation was chosen.
...
**Evidence (cont.):** L9b Plan 1 whole-branch review, C2, 2026-08-22 — the same blindness, but on a WRITE, and the stake was irreversible. `account_deletion_requests`' INSERT policy constrained only `user_id` and `status`, leaving `tier`, `execute_after` and `purge_after` free, and the request was written through the user's client — so `authenticated` had to keep the INSERT grant. Anyone holding a session token could POST straight to PostgREST with `execute_after: now()` and be erased within one 60-second scheduler tick, skipping the typed confirmation, the acknowledgement checkbox and the whole 7-day cancellation window. The full suite was green before and after; a mocked test cannot see a grant in either direction. Settled the same way as the certification case: a real auth user, a real signed-in JWT, a real PostgREST POST — **201 with both timestamps already in the past before the fix, `42501 permission denied` after**, plus a third probe proving the normal path (service-role insert → the user's own SELECT reads it back → another user's SELECT returns `[]`) still worked. **The generalisable part:** the reviewer deferred this finding twice, both times reasoning that a DB-level floor would put the grace-window fact in a second home. That reasoning is right about duplication and irrelevant to the threat: the fix that duplicates nothing is to remove the client's write path entirely, not to restate the rule in SQL. When "one fact, one home" is the argument for leaving a security control in application code alone, check whether the control can instead be made unreachable.
...
### L-034 — A test that mocks a context resolver to a fixed value cannot catch that resolver returning the wrong thing in production

**Rule:** Before mocking `getLocale()`, `getUser()`, or any function whose real value comes from framework wiring (middleware, route matcher, request scope), ask *does this code path actually run inside the wiring that supplies it* — never *is the test green with the mock*.
**Why:** Same shape as `L-005` in a different domain: `getLocale()` (`lib/i18n/server.ts`) resolves via an `X-NEXT-INTL-LOCALE` header the next-intl middleware sets — and `middleware.ts`'s matcher excludes `api`, so on the real request path for any Route Handler that header never exists and `getLocale()` silently returns `routing.defaultLocale` regardless of the caller's actual locale. A unit test mocking `@/lib/i18n/server` to return a fixed locale doesn't just fail to catch this — it mocks away the exact function whose real behavior needed proving, so the test is structurally incapable of failing no matter what the implementation does.
**Evidence:** `feat/email-notification-system` code review — `lib/data/account-deletion.ts::requestDeletion` called `getLocale()` from inside `app/api/user/deletion/route.ts` (an API route) to build a deletion-notification email's locale and cancel link; every English caller silently got a Vietnamese email with a `/vi/...` link. `lib/data/account-deletion.test.ts` mocked `getLocale` to return `"en"` and was green throughout. The repo already documented the underlying mechanism in `app/[locale]/(auth)/actions.ts`'s own comment ("deliberately excluded from the intl middleware … its own fallback is the unprefixed default locale") for a *different* route (`/auth/callback`) — the knowledge existed in the codebase but not at the call site that needed it. Fixed by forwarding `locale` explicitly in the validated request body instead of resolving it server-side, mirroring the pattern `(auth)/actions.ts` already uses for `emailRedirectTo`. The mutation check for the replacement test also failed once in the same family: a first version passed only `"vi"` (which **is** `routing.defaultLocale`), so hardcoding the bug back in would have left that test green too — parameterizing over both locales was what actually proved the value came from the caller.
**Applies to:** any Route Handler, background job, or server module invoked outside a page's middleware-covered request tree that calls a next-intl/auth/request-scoped resolver. Before trusting such a call, check `middleware.ts`'s matcher for an exclusion, the same way `L-005` checks a table's RLS policy before trusting a read.
...
**Evidence:** Lesson Workspace Plan A — 1 Critical + 4 Important, incl. an e2e spec still clicking a renamed nav label. · Token foundation `86328bc` — 5 Important, incl. `color-scheme: dark` never declared. · Screen-port workflow `7277ac1` — caught that the `(focus)` contract had removed the reduce-motion control, breaking `CLAUDE.md` §2 rule 4. · Plan C1 `bd7f574` — round 1 returned CHANGES REQUIRED and was right five for five, incl. a redirect swallowing `/api/videos`. · Screen Registry Phase 3 Stage 1 (2026-08-25) — **six per-task reviews returned two Minors between them, neither load-bearing: Task 5's review caught a `G2` comment saying "the two dates" over a pin that had grown to three (folded into Task 6's dispatch rather than a fix round), and Task 6's re-review caught the frame map's batch note disagreeing with the measured registry (fixed at `d37027f`). The whole-branch review then returned CHANGES REQUESTED with four Important.** The four share one shape that explains why no task review could have caught them: each was a sentence that was TRUE when its commit wrote it and was falsified by a later commit that never touched the line. A status memory said "NOTHING IMPLEMENTED" (true at the spec commit, false eight commits later); a header's hand-checkable invariant drifted past its own recipe; two documents' accounting of the same exclusion set diverged; and a test's NAME was inverted by a widening that kept the name's wording (`it("T3: figmaNodeId is present iff …")` over a body asserting `toBeNull()`, where the pre-branch name had read "is **not** repo-only"). A per-task diff cannot contain the contradiction, because the two halves live in different commits. **The generalisable test:** the whole-branch pass should read for sentences whose truth depends on a commit other than the one that wrote them.
...
**Evidence:** Screen-port workflow `7277ac1` — the fix restoring the reduce-motion control mounted it outside the nav, turning a 24px strip into a ~130px labelled rail on every authenticated screen. Resolved with a `compact` prop that `sr-only`s the caption. · Screen Registry Phase 1b — the re-review of `4db8e7b..HEAD` found no code defect but four documents outranking the truth, including the branch's own resume memory. · Screen Registry Phase 2a `01eb284`+`5c7a6cd` — a wave closing eleven findings introduced a fresh false statement of its own (a spec sentence enumerating which questions stay open was wrong in **both** directions — it listed one the file's own list had answered and omitted one still open) and left the single live line-citation it existed to fix pointing ~29 lines off target, because its own `+21` lines moved it. Closed at `bf344a4`. **Twice now the fix wave's defect has been the same shape: a claim written *about* a file the same commit was editing.** Check the wave's own output against the wave's own edits before closing it. · `certification-grants-hardening` (2026-08-19) — fired on every fix wave of one branch, over a comments-only diff. Wave 1 shipped a wrong verification recipe inside `L-001`, the entry whose subject is unverified success signals; wave 2 fixed the recipe and shipped four wrong qualifiers around it; wave 3 fixed those and got the signal's own description wrong. Each wave was caught by the review of the wave before it, which is the entire argument for this lesson. The pattern narrowed each time — substance, then scope words, then one scope word — which is what a converging review loop looks like from the inside. · Screen Registry Phase 3 Stage 1 `4043a74` — the wave whose entire purpose was removing stale claims wrote **"Nine commits on the branch"** into the status memory *while creating the tenth*, then appended "plus the prose fix wave that followed" to the list it had just called nine, contradicting itself inside one sentence. **This is the sharpest form of the shape L-012 keeps finding:** not a claim about a file the same commit edits, but a claim about the commit it is written inside — which is unverifiable at authoring time by construction, since the commit does not exist yet. The scoped re-review then found a **second** instance the controller's own sweep had missed. A wave that reports on its own branch state must name the range and the counting command (`L-002`), never a total.
...
**Evidence (cont.):** L9b Plan 1, C1, 2026-08-22 — the spec's tier table stated *"Learning data survives; the account is reopenable."* Nothing in the plan ever built a reopen path, and `close_account` bans the auth user for `876000h` with the only `liftBan` caller being the scheduler's failure handler. The claim propagated straight into shipped user-facing copy in BOTH locales — *"and you can come back"*, *"until you reopen your account"* — read at the exact moment a user decides whether to close their account. Six task reviews and two fix rounds passed over it, because every one of them checked the implementation against the spec and the implementation matched. **The part that generalises:** a spec sentence describing a CAPABILITY the product will have is indistinguishable, to every downstream reader, from one describing a capability it has — and the copywriter is the reader who turns it into a promise to the user. A spec that states a capability must be checkable against a named artifact (an endpoint, a route, a function with a caller), or it must be written in §14 as deferred.
**Applies to:** spec and plan authoring, and defect triage. The highest-yield spec audit is not "is this implemented?" but "is every capability sentence attached to something a grep can find?".
...
### L-015 — Never seed a subagent with a claim you have not verified — a proposition, not only a number

**Rule:** Treat every factual assertion in your own dispatch prompt as a claim you are making — figures, but equally "X will fail", "Y is caused by Z", "the set is A plus B". Verify it first, or instruct the agent to verify it and to report disagreement as a finding.
**Why:** The agent repeats it, and an unverified claim comes back looking independently confirmed — worse than stating nothing. A *proposition* is more dangerous than a figure: a wrong number often looks wrong, while a wrong causal or set claim reads as domain knowledge the agent has no standing to doubt.
**Evidence:** Plan C1 — an unmeasured "26" went into a code-reviewer's prompt and was echoed in its report. The round-2 dispatch instead told the reviewer to measure, and it promptly caught the next stale count. · Screen Registry Phase 3 Stage 1, 2026-08-25 (recorded at `cf6ab8e`) — **the controller seeded three false propositions, none of them a number.** (a) A dispatch and the plan both said a mutation "will also fail `R12` because there are now 14 keys" — false; the field had been backfilled onto every row in Task 1, so the mutation changed a *value*, not the key set, and `R12` correctly stayed green. The implementer caught it unprompted and said so. (b) A ruling asserted the full suite "exceeds the 600s cap" — false; it runs in ~72s and the real cause was output volume (`L-035`). (c) A fix-round message restated the 2026-08-23 frame batch's membership from the controller's memory, and it disagreed with that accounting's **single declared home** — the ✅ Registered note at the top of `docs/product/figma-frame-map.md`, which is the only place the batch is decomposed and which had already been reconciled against the measured registry. The arithmetic is deliberately not repeated here (`L-002`, `CLAUDE.md` §6): read the home. The implementer transcribed (c) faithfully and the resulting text had to be fixed twice. **What generalises:** (a) and (b) were caught, (c) was not — and the difference is that (a) and (b) were checkable from inside the task, while (c) required reading a document the agent had no reason to open. Seed only claims the receiving agent can falsify from its own working set; anything else must be handed over as "verify this, report disagreement".
**Applies to:** every dispatch prompt, and every message resuming an agent. See `L-002` for the same disease in documents, and `L-013` for the same disease in specs and plans.
...
### L-037 — One agent in flight at a time; an unblocked tool call is not an authorised one

**Rule:** Dispatch task N+1 only after task N's implementer has returned, its reviewer has returned, **and** the controller has read that review. After dispatching, the controller's next action is to wait — not to dispatch again.
**Why:** A reviewer's finding on task N can invalidate work task N+1 has already built on top of, and the per-task gate exists precisely to stop that. The failure mode is specific to permissive modes: when nothing prompts for confirmation, "the tool call went through" gets read as "this was allowed", and the controller starts the next task while the previous one is still being judged.
**Evidence:** Screen Registry Phase 3 Stage 1, 2026-08-25 (the controller rulings for that run are recorded in `.serena/memories/screen_registry_phase_3_run_state.md` § Stage 1 execution ledger, committed at `cf6ab8e`) — the user observed a task being started while the previous task's reviewer was still running, and named the cause exactly: auto-mode auto-answers *permission prompts* and says nothing about sequencing. Run strictly one-at-a-time afterwards, the branch's six tasks produced information each later dispatch needed and could not have had otherwise — the CRLF hazard (`L-036`), the reporter fix (`L-035`), and a T12/T13 interaction that would have read as a broken test to an agent dispatched in parallel.
**Applies to:** every subagent-driven run, and most sharply when no permission prompt is standing between dispatches. Distinct from `L-016`: that one is about *verifying* after a dispatch, this one is about not *overlapping* dispatches.
...
**Why:** Unrelated mechanisms in this environment — each one enumerated in the evidence below — make a search lie silently, and every one of them presents as "no matches", which reads as a clean sweep rather than as a broken command.
**Evidence:** Plan C1 — **`npx rg` is not ripgrep**: it installs an unrelated stub package and returns misleading output. · Screen Registry Phase 2b `dc51d36` — in this repo's Git Bash, **a `git grep` pattern beginning with `/` is mangled by MSYS path conversion** and matches nothing regardless of the truth. Measured three ways: `git grep -c "/jlpt" -- 'lib/supabase/*.ts'` → empty, exit 1; `MSYS_NO_PATHCONV=1 git grep -c "/jlpt" …` → correctly two files; `git grep -c "[/]jlpt" …` → the same two. This made two plan steps vacuous **by construction** — Task 3 Step 5 (`git grep "/api/jlpt"`, "expected: no output") and Task 7's G4 sweep. G4 was the artifact standing in for a guard the user had explicitly deferred *on condition that the sweep be run in its place*, so the one check bought with that ruling could not have failed. It was saved only because the implementer ran a control first and noticed the silence. · Screen Registry Phase 3 Stage 1, 2026-08-25 (recorded at `cf6ab8e`) — a further mechanism, and the dullest: **plain case sensitivity.** A controller sweep for a defect it had just fixed printed "0 occurrences (fixed)" — but the positive control in the same invocation also returned 0, because the control's pattern was lower-case and the text was capitalised. The empty result was discarded rather than believed, the sweep re-run with `-i`, the control fired, and the real sweep then returned a hit the first run would have hidden. **This is the case that argues the rule hardest:** the two exotic mechanisms above are memorable, so people guard against them; a case-sensitive grep is so ordinary that "0 occurrences" reads as an answer rather than as a question.
**Applies to:** any search from a shell. A leading `/` needs `MSYS_NO_PATHCONV=1` or a `[/]` character class. Put the control in the *same* invocation as the sweep, and read its result before reading the sweep's. Composes with `CLAUDE.md` §7: a sweep is a collection gathered by a pattern, so it needs a control that fires before an empty result means anything.
**Status:** Promotion review **triggered** by the evidence entry added 2026-08-25 (rule 3) and **parked, not run** — promotion is the user's call and no session has taken it. Park, not decline: unlike `L-016` and `L-023` this one *does* have a checkable invariant available ("an empty search result is reported only alongside a control that fired in the same invocation"), so the review has something to decide. Whoever runs it decides only whether that belongs in `CLAUDE.md` §7 beside the collection clause it already composes with.
...
### L-035 — The full vitest run dies of output volume, not of duration — use `--reporter=dot`

**Rule:** Run the whole suite as `npx vitest run --reporter=dot`. When a run is killed, measure the duration before theorising about timeouts.
**Why:** The default reporter streams per-test stdout for a suite whose tests deliberately log (the scheduler and AI-port tests print on every pass), and the volume alone gets the run killed. The failure looks exactly like a timeout — no summary line, no exit code — so the natural diagnosis is "too slow", which sends you to raise a limit that was never the constraint.
**Evidence:** Screen Registry Phase 3 Stage 1, 2026-08-25 (the false ruling and its correction are recorded in `.serena/memories/screen_registry_phase_3_run_state.md` § Stage 1 execution ledger, committed at `cf6ab8e`) — two full-suite runs were killed with no `EXIT=` line ever written. The controller ruled that the suite "exceeds the 600s cap" on a subagent's Bash tool and reassigned the run to itself. The rerun with `--reporter=dot` completed in **71.77s** — roughly one-eighth of the cap the ruling had blamed. The ruling's conclusion was harmless but its stated reason was false, and it was corrected rather than left to be inherited by later tasks (`L-012`). The separately true fact worth keeping: a subagent's Bash tool does cap at 600s, so any command genuinely approaching it belongs in the controller's seat or in a background run.
**Applies to:** every whole-suite run, and every diagnosis of a killed command — check duration before blaming a timeout.

### L-036 — A script that writes a repo file bypasses git's line-ending normalisation

**Rule:** Edit tracked files with the editing tools. If a script must write one, verify afterwards with `git hash-object <file>` against `git rev-parse HEAD:<file>` — and if the hashes match while `git status` still says modified, `git checkout -- <file>` to normalise.
**Why:** This repo normalises to CRLF on checkout. A `node`/`sed` script writes LF, so the file's *content* is identical as far as `git diff` and `git hash-object` are concerned while `git status` reports it modified indefinitely — and `git update-index --refresh` does not clear it. The danger is not the noise: the next task that runs `git add -A` commits a whole-file line-ending change that buries the real diff and makes review of that commit worthless.
**Evidence:** Screen Registry Phase 3 Stage 1, 2026-08-25 — a backfill script added one field to every registry entry (81 of them at `467b7c1`, the commit that carries the backfill; count at any commit with `grep -c '^  {$' lib/product/screen-registry.ts`, `L-002`) and left `lib/product/screen-registry.ts` permanently ` M`. An implementer reported it as "a stale mtime in git's stat cache"; the real cause was the LF/CRLF flip, and the file at that moment was byte-identical to `HEAD` by hash. Left unfixed it would have landed inside Task 5 (`dc917dc`), the one task that edits that same file heavily — turning that commit's 181-line reviewable diff into a whole-file rewrite.
**Applies to:** any scripted edit of a tracked file on this Windows checkout.
