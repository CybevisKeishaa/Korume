...
import { z } from "zod";
import { submitJlptTest } from "@/lib/data/jlpt";
import { jlptSubmitSchema } from "@/lib/validation/jlpt";
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const parsed = jlptSubmitSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const result = await submitJlptTest(params.id, parsed.data);
  if (!result.ok) {
