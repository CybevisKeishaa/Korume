 ...
 * `certification_tests`/`certification_questions` RLS already scopes `select`
 * to the `authenticated` role, so a signed-out request is blocked by RLS
 * itself (empty result) rather than an explicit 401 here. Submissions and the
