...
export function JlptQuestionNavigator({
  ...
  return (
    <nav aria-label={t("navigator.aria")} className="grid grid-cols-6 gap-2 sm:grid-cols-10">
      {Array.from({ length: count }, (_, i) => {
        const answered = answeredIndexes.has(i);
        const current = i === currentIndex;
        const ariaLabel =
          t("navigator.item", { index: i + 1 }) +
          (current ? `, ${t("navigator.current")}` : "") +
          (answered ? `, ${t("navigator.answered")}` : `, ${t("navigator.notAnswered")}`);
        return (
          <button
            key={i}
            type="button"
